...
class DeployedTraineesChart extends ChartWidget
{
    protected static ?string $heading = 'Deployed/To be Deployed Trainees';
    ...
    protected function getData(): array
    {
        ...
        $data = Trend::query(Trainee::deployedWhen('past'))->dateColumn('date_deployment');
        $data_upcoming = collect([]);

        $data_upcoming_chart = [
            'label' => 'Upcoming trainees to be deployed',
            'data' => [],
        ...
        if (in_array($this->filter, ['last_week', 'this_week', 'last_month', 'this_month'])) {
            if ($this->filter == 'last_week') {
                ...
                $data_upcoming = $this->append_base_query()->perDay();
            }
            ...
            if ($this->filter == 'last_year') {
                ...
                $data_upcoming = $this->append_base_query()->perMonth();
            }
        ...
        $data = $data->count();
        $data_upcoming = $data_upcoming->count();
        ...
        if ($data_upcoming) {
            $data_upcoming_chart['data'] = $data_upcoming->map(fn (TrendValue $value) => $value->aggregate);
        }

        return [
            'datasets' => [
                [
                    'label' => 'Deployed trainees',
                    'data' => $data->map(fn (TrendValue $value) => $value->aggregate),
                    'borderColor' => '#559ccf', // primary-400
                    'backgroundColor' => '#f3f8fc', // primary-50
                ],
                $data_upcoming_chart,
            ],
            'labels' => $labels
    ...
    {
        return 'The number of deployed/upcoming trainees to be deployed ' .$this->extra_description;
    }
    ...
    protected function append_base_query(): Trend
    {
        $trend_query = Trend::query(Trainee::deployedWhen('future')
            ->whereBetween('date_deployment', [$this->start_value->toDate(), $this->end_value->toDate()])
        )
            ->dateColumn('date_deployment')
            ->between(start: $this->start_value, end: $this->end_value);
